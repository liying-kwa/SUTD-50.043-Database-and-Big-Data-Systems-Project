...
def book(asin):
    ...
    if book == None:
        if 'logged-in' in session:
            logs_db.insert_one({"user": session['user']['email'], "action":"view", "response": 404, "content": asin, "datetime": datetime.datetime.now()})
        return render_template('not_found.html'), 404
    if 'logged-in' in session:
        logs_db.insert_one({"user": session['user']['email'], "action":"view", "response": 200, "content": book['title'], "datetime": datetime.datetime.now()})
...
def add_review(asin):
    ...
    if 'logged-in' in session:
        ...
        logs_db.insert_one({"user": session['user']['email'], "action":"add_review","response": 200, "content": reviewText, "datetime": datetime.datetime.now()})
...
def add_book():
    if 'logged-in' in session and check_admin():
        if request.method == "POST":
            ...
            logs_db.insert_one({"user": session['user']['email'], "action":"add_book", "response": 200,  "content": title, "datetime": datetime.datetime.now()})
...
def del_book(asin):
    if 'logged-in' in session and check_admin():
        ...
        logs_db.insert_one({"user": session['user']['email'], "action":"remove_book", "response": 200,"content": asin, "datetime": datetime.datetime.now()})
        return redirect(url_for('index'))
    else:
        if 'logged-in' in session:
            logs_db.insert_one({"user": session['user']['email'], "action":"remove_book", "response": 401, "content": asin, "datetime": datetime.datetime.now()})
        return redirect(url_for('unauthorised'))
...
def search():
    ...
    if 'logged-in' in session:
        logs_db.insert_one({"user": session['user']['email'], "action":"search", "response": 200,  "content": search_input, "datetime": datetime.datetime.now()})
